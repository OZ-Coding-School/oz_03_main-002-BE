...
class GoogleSocialAuthSerializer(serializers.Serializer):
    ...
    def validate_auth_token(self, auth_token):
        ...
        try:
            idinfo = id_token.verify_oauth2_token(
                auth_token, google_requests.Request(), os.environ.get("GOOGLE_CLIENT_ID")
            )
    ...
    def complete_login(self, request):
        ...
        userinfo_response = request_session.get('https://www.googleapis.com/oauth2/v3/userinfo')
        userinfo_response.raise_for_status()
        data = userinfo_response.json()

        try:
            # 기존 사용자 확인
            social_account = SocialAccount.objects.get(provider='google', uid=data['sub'])
