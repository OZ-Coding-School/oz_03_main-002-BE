...
DEBUG = os.environ.get('DEBUG', default=False)  # 개발 모드 (True) 또는 배포 모드 (False)
...
TEMPLATES = [
    {
        ...
        'DIRS': [BASE_DIR / 'templates'],  # 템플릿 디렉토리 설정 (사용하지 않을 경우 생략 가능)
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
...
DATABASES = {
    'default': {
        ...
        'PASSWORD': os.environ.get('RDS_PASSWORD'),  # 데이터베이스 비밀번호 (환경 변수 사용)
