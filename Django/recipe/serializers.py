...
class CookingAttributeSerializer(serializers.ModelSerializer):
    ...
    situation = serializers.PrimaryKeyRelatedField(queryset=CookingSituation.objects.all())
    main_ingre = serializers.PrimaryKeyRelatedField(queryset=CookingMainIngre.objects.all())
...
class RecipeSerializer(serializers.ModelSerializer):
    nick_name = serializers.CharField(source='nick_name.nick_name')  # nick_name.nick_name으로 수정
    attribute = CookingAttributeSerializer()
    ingredients = RecipeIngredientListSerializer(
        source='recipeingredientlist_set', many=True, read_only=True, required=False  # ingredients 필드를 optional로 설정
    )
    ...
    def create(self, validated_data):
        ...
        situation, _ = CookingSituation.objects.get_or_create(name=attribute_data['situation'])
        main_ingre, _ = CookingMainIngre.objects.get_or_create(name=attribute_data['main_ingre'])
        type, _ = CookingType.objects.get_or_create(name=attribute_data['type'])

        cooking_attribute = CookingAttribute.objects.create(
            name=name, method=method, situation=situation, main_ingre=main_ingre, type=type
        )
        ...
        for ingredient_data in ingredients_data:
            ...
            RecipeIngredientList.objects.create(recipe=recipe, ingredient=ingredient, **ingredient_data)
