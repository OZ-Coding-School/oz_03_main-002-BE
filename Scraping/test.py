...
for num in data:
    ...
    if (
        data[num]["thumbnail_url"] == None
        or data[num]["ingre_list"] == None
        or data[num]["detail_recipes"] == None
    ):
        cnt += 1
        try:
            url = data[num]["URL"]
            ...
            thumbnail_url = thumbnail_element.find_element(
                By.TAG_NAME, "img"
            ).get_attribute("src")
            ...
            recipe_elements = driver.find_elements(
                By.CSS_SELECTOR, ".step_list.st_thumb li"
            )
            for i, element in enumerate(recipe_elements):
                detail = {}
                detail_recipe = element.find_element(
                    By.CSS_SELECTOR, ".step_list_txt_cont"
                )
                detail["recipe"] = detail_recipe.text

                try:
                    detail_tip = element.find_element(
                        By.CSS_SELECTOR, ".step_list_txt_tip"
                    )
                ...
                try:
                    detail_img = element.find_element(
                        By.CSS_SELECTOR, ".step_list_txt_pic img"
                    )
